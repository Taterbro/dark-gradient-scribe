...
const NoteSidebar: React.FC<NoteSidebarProps> = ({
  ...
  return (
    <div className={`fixed top-0 right-0 h-full transition-all duration-300 ${isSidebarOpen ? 'w-64' : 'w-10'} bg-black/30 border-l border-white/10 flex flex-col backdrop-blur-sm z-10`}>
      {/* Sidebar toggle */}
      <button 
            ...
            {notes.length > 0 ? (
              <ul>
                {notes.map(note => (
                  <li 
                    key={note.id} 
                    className={`p-3 border-b border-white/5 cursor-pointer hover:bg-white/5 transition-colors ${activeNoteId === note.id ? 'bg-white/10' : ''}`}
                  >
                    <div 
